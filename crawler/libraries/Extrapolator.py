...
def get_database_record(date,hour):
    ...
    #dev_feat,
    real_AQI,pred_AQI=Extrapolate(grid,df.drop(columns=['Device']),model)
    ...
    return (
            df.Device.values.tolist(), #sending active devices
            database_record #sending Extracted information (database record to store)
    )